...
DEFAULT_EVENTS_CFG = {
    ...
    "stimulus_input_mode": "event",
    "require_light_on": False,
    "require_light_off": False,
    "derive_events_from_interval": True,
}

DEFAULT_INTERVAL_CFG = {
...
class NexPackageBackend:
    def __init__(self, config: dict, paths: dict, logger):
        ...
        events_cfg = self._events_cfg()
        self.current_interval_name = events_cfg.get("interval_name", "Light_Interval")
        self.current_event_on_name = events_cfg["event_on_name"]
        self.current_event_off_name = events_cfg["event_off_name"]

    def _events_cfg(self) -> dict:
        return {
            **DEFAULT_EVENTS_CFG,
            **self.config.get("neuroexplorer", {}).get("events", {}),
        }

    def _interval_cfg(self) -> dict:
        return {
            **DEFAULT_INTERVAL_CFG,
            **self.config.get("neuroexplorer", {}).get("interval", {}),
        }
    ...
    def ensure_events(self, file_id, light_on_times, light_off_times) -> None:
        ...
        if events_cfg.get("stimulus_input_mode", "event") == "interval":
            interval_cfg = self._interval_cfg()
        ...
        on_name = events_cfg["event_on_name"]
        off_name = events_cfg["event_off_name"]
        if on_name in event_names and (off_name in event_names or not events_cfg["require_light_off"]):
            self.logger.log(
                "neuroexplorer_nex_backend",
                str(file_id),
                "",
    ...
    def validate_required_events(self) -> None:
        ...
        events_cfg = self._events_cfg()
        required = []
        if events_cfg["require_light_on"]:
            required.append(self.current_event_on_name)
        if events_cfg["require_light_off"]:
